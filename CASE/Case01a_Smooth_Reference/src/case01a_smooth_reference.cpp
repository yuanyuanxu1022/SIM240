  ...
  const std::filesystem::path& path)
{
  ...
  for (int iC = 0; iC < lattice.getLoadBalancer().size(); ++iC) {
    ...
    block.forCoreSpatialLocations([&](LatticeR<3> latticeR) {
      ...
      const T rho = block.get(latticeR).computeRho();
      const T latticePressure =
        (rho - T{1}) / descriptors::invCs2<T, DESCRIPTOR>();
      sums[iy] += converter.getPhysPressure(latticePressure);
      ++counts[iy];
    });
  }

  T domainMean = 0.0;
  long long domainCount = 0;
  for (int iy = 0; iy < ny; ++iy) {
    domainMean += sums[iy];
    domainCount += counts[iy];
  }
  domainMean /= domainCount;

  std::ofstream out(path);
  out << std::setprecision(17)
      << "y_m,y_nm,mean_gauge_pressure_Pa,pressure_minus_domain_mean_Pa,sample_count\n";
  for (int iy = 0; iy < ny; ++iy) {
    const T pressure = sums[iy] / counts[iy];
    const T y = (iy + T{0.5}) * dx;
    out << y << ',' << y * T{1.0e9} << ',' << pressure << ','
        << pressure - domainMean << ',' << counts[iy] << '\n';
  }
}
...
{
  lattice.setProcessingContext(ProcessingContext::Evaluation);
  singleton::directories().setVtkOutDir(
    (outputDirectory / "vtk").string() + "/");
  SuperVTMwriter3D<T> writer("case01a_smooth_reference");
...
int main(int argc, char* argv[])
{
  ...
  if (mode == "precheck") {
    ...
    result << std::setprecision(17) << std::boolalpha
      << "run_scope=Case01a_Smooth_Reference_H75_precheck\n"
  ...
  record(0, initial);
  std::cout << "run_scope=Case01a_Smooth_Reference_H75_dx5_tau1p7\n"
  ...
  writePressureProfile(
    lattice, geometry, converter, outputDirectory / "pressure_profile.csv");
  writeFinalVtk(lattice, converter, outputDirectory, finalStep);
  ...
  std::ofstream result(outputDirectory / "result.txt");
  result << std::setprecision(17) << std::boolalpha
    << "run_scope=Case01a_Smooth_Reference_H75_dx5_tau1p7\n"
