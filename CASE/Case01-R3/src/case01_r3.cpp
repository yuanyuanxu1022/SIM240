...
constexpr T dt = 4.1666666666666667e-12;
constexpr T targetTau = 1.0;
  ...
  const std::filesystem::path& path)
{
  ...
  out << std::setprecision(17)
      << "z_m,z_nm,u_y_numerical_m_s,u_y_analytic_m_s,point_relative_error,offset_m_s\n";
  for (int iz = 0; iz < nz; ++iz) {
    ...
    out << z << ',' << z * 1.0e9 << ',' << numerical << ',' << analytic
        << ',' << offsets[iz] / analytic << ',' << offsets[iz] << '\n';
  }
...
int main(int argc, char* argv[])
{
  ...
  const T converterTau = converter.getLatticeRelaxationTime();
  const T reconstructedNu = (T{1} / T{3}) * (converterTau - T{0.5}) * dx * dx / dt;
  const T converterTolerance = 64.0 * std::numeric_limits<T>::epsilon();
  const bool converterPass =
    std::abs(converterTau - targetTau) <= converterTolerance
    && std::abs(reconstructedNu - nuPhys)
         <= 128.0 * std::numeric_limits<T>::epsilon() * nuPhys
    && std::abs(lx / dx - nx) <= converterTolerance
    && std::abs(ly / dx - ny) <= converterTolerance
    && std::abs(height / dx - nz) <= converterTolerance;
  ...
  if (mode == "precheck") {
    ...
    result << std::setprecision(17) << std::boolalpha
      << "run_scope=Case01_R3_dx5_tau1p0_precheck\n"
      << "formal_simulation=false\n"
      << "collide_and_stream_calls=0\n"
      << "geometry_PASS=" << geometryPass << '\n'
      << "converter_PASS=" << converterPass << '\n'
      << "converter_tau=" << converterTau << '\n'
      << "target_tau=" << targetTau << '\n'
      << "reconstructed_nu_m2_s=" << reconstructedNu << '\n'
  ...
  auto record = [&](int step, const Measurement& state) {
    ...
    diagnostics << step << ',' << step * dt << ',' << state.fluidNodes << ','
      << state.fluidVolume << ',' << state.fluidMass << ',' << massRelative << ','
  ...
  record(0, initial);
  std::cout << "run_scope=Case01_R3_dx5_tau1p0_a1e5\n"
  ...
  std::ofstream result(outputDirectory / "result.txt");
  result << std::setprecision(17) << std::boolalpha
    << "run_scope=Case01_R3_dx5_tau1p0_a1e5\n"
