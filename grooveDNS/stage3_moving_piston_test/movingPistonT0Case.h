#ifndef STAGE3_FREE_SPHERE_CASE_H
#define STAGE3_FREE_SPHERE_CASE_H
...
struct DT_SCALE : public descriptors::FIELD_BASE<1> { };
struct CHAR_LATTICE_VELOCITY : public descriptors::FIELD_BASE<1> { };
}
...
inline void prepareMinimalLattice(Stage3Case& c)
{
  ...
  lattice.setUnitConverter<UnitConverterFromResolutionAndLatticeVelocity<T,DESCRIPTOR>>(
    ...
    T{p.get<parameters::CHAR_LATTICE_VELOCITY>()},
  ...
  const std::array<T,3> center{T(0.5)*e[0],T(0.5)*e[1],T(0.5)*e[2]};
  ExactSphereVolumeField3D<T> typeField(center,radius,dx,{0,1,2});
  ExactSphereVolumeField3D<T> fractionField(center,radius,dx,{0,0.5,1});
