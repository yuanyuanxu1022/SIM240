...
bool cvAnalyticalSolid(T x,T z,T h)
{
  x=std::fmod(x,Lx);if(x<0)x+=Lx;
  return z<0 || punch(x,z,h);
}

bool cvPhysicalFluidCenter(const Vector<T,3>& r,T h)
{
  return r[2]>=0 && !cvAnalyticalSolid(r[0],r[2],h);
}

template<class G>
long long cvGhostFluidCount(G& geometry,T h)
{
  long long count=0;auto& g=geometry.getBlockGeometry(0);
  g.forCoreSpatialLocations([&](LatticeR<3> p){
    if(isFluidMaterial(g.getMaterial(p))&&!cvPhysicalFluidCenter(g.getPhysR(p),h))++count;
  });
  return count;
}

template<class L,class G>
std::array<int,6> cvUpdatePeriodicLinks(L& lattice,G& geometry,T h,T uWall)
{
  int active=0,invalid=0,recovered=0,falseSolid=0,virtualLinks=0,activeCells=0;
  ...
  block.forCoreSpatialLocations([&](LatticeR<3> p) {
    ...
    if(!cvPhysicalFluidCenter(r,h))return;
    ++activeCells;
    for(int i=1;i<D::q;++i) {
      ...
      const int mappedMaterial=crosses?g.getMaterial(mapped):rawMaterial;
      const auto neighborR=g.getPhysR(mapped);
      const bool analyticalSolid=cvAnalyticalSolid(neighborR[0],neighborR[2],h);
      if(crosses&&(mappedMaterial==2||mappedMaterial==3)
         &&rawMaterial!=2&&rawMaterial!=3) ++recovered;
      if(crosses&&(rawMaterial==2||rawMaterial==3)
         &&mappedMaterial!=2&&mappedMaterial!=3) ++falseSolid;
      if(!analyticalSolid)continue;
      if(mappedMaterial!=2&&mappedMaterial!=3)++virtualLinks;
      T q=.5,velocityCoefficient=0;
      const bool punchSide=neighborR[2]>=0;
      {
        T lo=0,hi=1;
        for(int k=0;k<50;++k) {
          const T a=(lo+hi)/2;
          T x=r[0]+a*dx*c[0];x=std::fmod(x,Lx);if(x<0)x+=Lx;
          if(cvAnalyticalSolid(x,r[2]+a*dx*c[2],h))hi=a;else lo=a;
        }
        q=(lo+hi)/2;
        if(punchSide)velocityCoefficient=c[2]*uWall*dt/dx;
      }
      if(!(q>=0&&q<=1)) ++invalid;
      else {
  ...
  lattice.communicate();return {active,invalid,recovered,falseSolid,virtualLinks,activeCells};
}
...
CVStats cvStats(L& lattice,G& geometry,const UnitConverter<T,D>& converter,T h)
{
  ...
  g.forCoreSpatialLocations([&](LatticeR<3> p) {
    ...
    const T alpha=cvAlpha(r,h);
    s.geomMass+=alpha*rho*rhoPhys*dx*dx*dx;
    s.finite&=std::isfinite(rho)&&std::isfinite(alpha);
    ...
    if(speed>s.maxU){s.maxU=speed;s.maxIx=p[0];s.maxIy=p[1];s.maxIz=p[2];s.maxMaterial=material;}
...
template<class L,class G>
CVPopulationFlux cvPopulationFlux(L& lattice,G& geometry,T h)
{
  ...
  g.forCoreSpatialLocations([&](LatticeR<3> p) {
    ...
    if(!cvPhysicalFluidCenter(g.getPhysR(p),h))return;
    auto cell=block.get(p);
    for(int i=0;i<D::q;++i) {
...
template<class L,class G>
void cvWriteActivePressure(const std::filesystem::path& path,L& lattice,G& geometry,
                           const UnitConverter<T,D>& converter,T h)
{
  std::ofstream out(path);out<<std::setprecision(17)
    <<"ix,iy,iz,x_nm,y_nm,z_nm,material,rho,physical_pressure_Pa\n";
  auto& block=lattice.getBlock(0);auto& g=geometry.getBlockGeometry(0);
  g.forCoreSpatialLocations([&](LatticeR<3> p){
    const int material=g.getMaterial(p);const auto r=g.getPhysR(p);
    if(!isFluidMaterial(material)||!cvPhysicalFluidCenter(r,h))return;
...
int cvRun(bool closed)
{
  const std::string runId=closed
    ? "step5_4_closed_not_authorized"
    : "step5_4_virtual_link_full10nm_v1_20260908";
  ...
  try {
    ...
    cvWriteActivePressure(outDir/"pressure_initial.csv",lattice,geometry,
                          converter,75e-9);
    ...
    int maxMachStep=0,maxMachIx=0,maxMachIy=0,maxMachIz=0,maxMachMaterial=0;
    ...
    std::ofstream linkHistory(outDir/"dynamic_link_history.csv");
    linkHistory<<"step,h_nm,active_links,delta_active_links,virtual_fluid_storage_links,"
                 "physical_active_cells,ghost_fluid_storage_cells,invalid_links,"
                 "false_periodic_solid_links\n";
    int previousActiveLinks=-1,maxVirtualLinks=0,maxGhostCells=0;
    for(int step=1;step<=cvSteps;++step) {
      ...
      const int ghostCells=cvGhostFluidCount(geometry,h);
      linkHistory<<step<<','<<std::setprecision(17)<<h*1e9<<','<<links[0]<<','
        <<(previousActiveLinks<0?0:links[0]-previousActiveLinks)<<','<<links[4]<<','
        <<links[5]<<','<<ghostCells<<','<<links[1]<<','<<links[3]<<'\n';
      previousActiveLinks=links[0];maxVirtualLinks=std::max(maxVirtualLinks,links[4]);
      maxGhostCells=std::max(maxGhostCells,ghostCells);
      lattice.collide();
      const auto popStep=closed?CVPopulationFlux{}:cvPopulationFlux(lattice,geometry,h);
      lattice.AndStream();
      ...
      const T currentMach=current.maxU/std::sqrt(T(1)/3);
      if(currentMach>maxMach){maxMach=currentMach;maxMachStep=step;
        maxMachIx=current.maxIx;maxMachIy=current.maxIy;maxMachIz=current.maxIz;
        maxMachMaterial=current.maxMaterial;}
    ...
    const bool pass=completed==cvSteps&&finite&&maxMach<=machLimit
      &&minRho>=rhoMinLimit&&maxRho<=rhoMaxLimit&&maxIllegal==0
      &&maxMaterialChanges==0&&unchanged&&maxAbsRGeom<=cvResidualLimit;
    cvWriteActivePressure(outDir/"pressure_final.csv",lattice,geometry,converter,
                          hAt(completed,true));
    ...
    result<<std::setprecision(17)<<std::boolalpha
      ...
      <<"\nmax_Mach_step="<<maxMachStep<<"\nmax_Mach_cell="<<maxMachIx<<','
      <<maxMachIy<<','<<maxMachIz<<"\nmax_Mach_material="<<maxMachMaterial
      ...
      <<"\nmax_virtual_fluid_storage_links="<<maxVirtualLinks
      <<"\nmax_ghost_fluid_storage_cells="<<maxGhostCells
    ...
    record<<"run_id="<<runId<<"\ncommand=mpirun -np 1 "
      <<"./explicit_piston_virtual_link_moving_geometry "
      <<(closed?"M2":"C")<<"\nsteps=10000\nmaterial_conversion_enabled=false\n"
      <<"dynamic_virtual_wall_links=true\nghost_fluid_storage=true\n"
      <<"exit_code="<<(pass?0:3)<<'\n';
