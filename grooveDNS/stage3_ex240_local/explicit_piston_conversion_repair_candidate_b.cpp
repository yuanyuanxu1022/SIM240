...
// Candidate B keeps Candidate A's frozen affected region. At alpha ~= 0.5,
// eight vertical receivers
  ...
  std::vector<SCCellSnapshot>& receiverAfter)
{
  ...
  transferError=0;
  using MomentIncrement=std::array<T,4>;
  std::map<std::tuple<int,int,int>,MomentIncrement> increments;
  for (auto& source : event) {
    ...
    std::array<T,3> sourceMomentum{};
    for (int i=0; i<D::q; ++i) {
      const T fullPopulation=populationScale
        *(source.f[i]+descriptors::t<T,D>(i));
      for (int iD=0; iD<3; ++iD) {
        sourceMomentum[iD]+=fullPopulation*descriptors::c<D>(i,iD);
      }
    }
    const T intended=alphaCurrent*source.rho;
    transferredMassLattice += intended;
    for (int iD=0; iD<3; ++iD) {
      transferredMomentum[iD] += alphaCurrent*sourceMomentum[iD];
    }
    for (const auto& q : receivers) {
      auto& increment=increments[{q[0],q[1],q[2]}];
      increment[0]+=intended/receivers.size();
      for (int iD=0; iD<3; ++iD) {
        increment[iD+1]+=alphaCurrent*sourceMomentum[iD]/receivers.size();
      }
    }
  }

  // Reconstruct each affected material-1 cell once. Existing local rho and
  // momentum plus all assigned increments define the target moments; only the
  // non-equilibrium part is discarded.
  for (const auto& [key,increment] : increments) {
    LatticeR<3> q{std::get<0>(key),std::get<1>(key),std::get<2>(key)};
    auto cell=block.get(q);
    T rhoBefore=1;
    std::array<T,3> momentumBefore{};
    for (int i=0; i<D::q; ++i) {
      const T fullPopulation=cell[i]+descriptors::t<T,D>(i);
      rhoBefore+=cell[i];
      for (int iD=0; iD<3; ++iD) {
        momentumBefore[iD]+=fullPopulation*descriptors::c<D>(i,iD);
      }
    }
    const T rhoTarget=rhoBefore+increment[0];
    T uTarget[3]{};
    for (int iD=0; iD<3; ++iD) {
      uTarget[iD]=(momentumBefore[iD]+increment[iD+1])/rhoTarget;
    }
    cell.iniEquilibrium(rhoTarget,uTarget);
    T rhoAfter=1;
    for (int i=0; i<D::q; ++i) rhoAfter+=cell[i];
    transferError+=(rhoAfter-rhoBefore)-increment[0];
  }
...
int scRun()
{
  const std::string runId="conversion_repair_B_equilibrium_column8_v1_20260908";
  ...
  try {
    ...
    result << std::setprecision(17) << std::boolalpha
      << "run_id=" << runId << "\nPASS=" << pass
      << "\nrepair_choice=B_local_rho_u_equilibrium_reconstruction"
      ...
      << "\naffected_cells_reconstructed_to_local_equilibrium=true"
    ...
    record << "run_id=" << runId
      << "\ncommand=mpirun -np 1 ./explicit_piston_conversion_repair_candidate_b"
      << "\nsource=explicit_piston_conversion_repair_candidate_b.cpp"
