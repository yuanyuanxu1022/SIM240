...
Stats computeStats(L& lattice,G& geometry,const UnitConverter<T,D>& converter,T h)
{
  ...
  for(int iC=0;iC<lattice.getLoadBalancer().size();++iC) {
    ...
    g.forCoreSpatialLocations([&](LatticeR<3> p) {
      ...
      const T rho=cell.computeRho();const T a=alpha(r,h);
      if(a>0) {
        s.geomMass+=a*rho*rhoPhys*dx*dx*dx;
        s.finite&=std::isfinite(rho)&&std::isfinite(a);
      }
...
int main(int argc,char** argv)
{
  ...
  const std::string runId="first_material_conversion_0to3nm_v3_20260909";
  ...
  try {
    ...
    history<<std::setprecision(17)<<std::boolalpha
      <<"step,time_s,h_nm,displacement_nm,event_f2s,event_s2f,cumulative_f2s,"
        ...
        "periodic_mapping_mismatches,dynamics_errors,active_fluid_mass_kg,M_geom_kg,"
        "cumulative_outward_mass_kg,R_geom_relative,rho_min,rho_max,max_Mach,max_ix,"
    ...
    T cumulativeOut=0,maxAbsResidual=0,globalMinRho=initial.rhoMin;
    ...
    for(int step=1;step<=lastStep;++step) {
      ...
      if(!tx.fluidToSolid.empty())residualAfterEvent=residual;
      ...
      history<<step<<','<<step*dt<<','<<hNew*1e9<<','<<(75e-9-hNew)*1e9<<','
        ...
        <<','<<current.activeMass<<','<<current.geomMass<<','<<cumulativeOut<<','<<residual
    ...
    manifest<<"model=SIM-EC1XT240\nrun_id="<<runId
      <<"\ncommand=mpirun -np 1 ./first_material_conversion_test\n"
