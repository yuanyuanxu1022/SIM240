  ...
  std::vector<SCCellSnapshot>& receiverAfter)
{
  ...
  const std::array<Vector<int,3>,5> offsets{
    Vector<int,3>{0,0,-1}, Vector<int,3>{1,0,-1},
    Vector<int,3>{-1,0,-1}, Vector<int,3>{0,1,-1},
    Vector<int,3>{0,-1,-1}
  };
  auto receiversFor=[&](const SCCellSnapshot& source) {
    std::vector<LatticeR<3>> receivers;
    for (const auto& d : offsets) {
      LatticeR<3> q=source.p+d;
      q[0]=cvWrapX(q[0]);
      q[1]=scWrapYOrInvalid(q[1]);
      if (q[1]<0) continue;
      const auto qr=g.getPhysR(q);
      if (isFluidMaterial(g.getMaterial(q)) && scMaterialAt(qr,hCurrent)!=3) {
  ...
  for (auto& source : event) {
    ...
    for (const auto& q : receivers) {
      ...
      for (int i=0; i<D::q; ++i) {
        ...
        destination[i] += alphaCurrent*fullPopulation/receivers.size();
      }
    }
    T afterRhoSum=0;
    for (const auto& q : receivers) {
    ...
    for (int iD=0; iD<3; ++iD) {
      transferredMomentum[iD] += alphaCurrent*scMomentum(source,iD);
    }
  }
...
int scRun()
{
  const std::string runId="single_conversion_ledger_v2_20260908";
  ...
  try {
    ...
    record << "run_id=" << runId
      << "\ncommand=mpirun -np 1 ./explicit_piston_single_conversion_ledger"
      << "\nsource=explicit_piston_single_conversion_ledger.cpp"
