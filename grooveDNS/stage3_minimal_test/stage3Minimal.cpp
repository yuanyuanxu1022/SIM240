...
void writeVtk(Stage3Case& c,std::size_t iT) {
  ...
  SuperVTMwriter3D<T> writer("stage3Minimal");
...
int main(int argc,char** argv) {
  ...
  if (singleton::mpi().getSize()!=1) {
    std::cerr << "stage3Minimal currently requires one MPI rank\n"; return 2;
  }
  Options opt;
  ...
  const T analytic=T(2)/T(3)*std::acos(T(-1))*radius*radius*radius;
  ...
  init<<std::scientific<<std::setprecision(15)
      <<"analytic_hemisphere_volume_m3 = "<<analytic<<"\n"
